...
class MatplotlibWidget(QWidget):
    ...
    def plot_graph(self, datafile=None):
        ...
        # Calcul de la FFT et de la phase
            fft_data = np.fft.fft(y)
            phase = np.unwrap(np.angle(fft_data))
            freq = np.fft.fftfreq(len(y), d=(x[1] - x[0]))

        # Filtrer pour ne garder que les fréquences positives
            pos_mask = freq >= 0
            freq = freq[pos_mask]
...
class MyWindow(QMainWindow):
    ...
    def create_menubar(self):
        ...
        material = QAction("matériau", self)
        hardware.triggered.connect(self.hardware)
        material.triggered.connect(self.material)
        file_menu.addAction(hardware)
        file_menu.addAction(material)
